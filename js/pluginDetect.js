...
[ isMinVersion getVersion hasMimeType getInfo onDetectionDone onWindowLoaded ]
...
(function () {
	...
	var f = {
		...
		setPluginStatus: function (r, p, s) {
			...
			q.getVersionDone = q.installed == 1 || q.installed == -1 || q.instance.hasRun ? 1 : 0;
		},
		getVersion: function (t, q) {
			var u = this,
				s, p = null,
				r;
			if ((!s || j.dbug) && u.nav.query().installed) {
				s = 1
			}
			if ((!p || j.dbug) && u.nav.query().version) {
				p = u.nav.version
			}
			if ((!s || j.dbug) && u.axo.query().installed) {
				s = 1
			}
			if ((!p || j.dbug) && u.axo.query().version) {
				p = u.axo.version
			...
			if (!p || j.dbug) {
				r = u.codebase.isMin(t);
				if (r) {
					u.setPluginStatus(0, 0, r);
					return
				}
			}
			if (!p || j.dbug) {
			...
			if ((!p && q) || j.dbug) {
				r = u.instance.query().version;
				if (r) {
					s = 1;
					p = r
				}
			}
			u.setPluginStatus(s, p, 0)
		},
		nav: {
			hasRun: 0,
			installed: 0,
		...
		instance: {
			...
			isDisabled: function () {
				...
				if (q.hasRun) {} else {
					if (j.dbug) {
						...
						if (f.nav.installed && j.hasMimeType(q.mimeType)) {
							p = 0
						}
					}
				}
			...
			query: function () {
				...
				p.HTML = j.DOM.insert("object", ["type", p.mimeType], ["autoplay", "no", "loop", "no", "volume", "0"], s, f);
				u = p.HTML.obj();
				if (u) {
					try {
						if (!r) {
							r = j.getNum(u.VersionInfo)
						}
					} catch (t) {}
					try {
						if (!r) {
	...
	var l = {
		...
		setPluginStatus: function () {
			...
			if (p.verify && p.verify.isEnabled()) {
				...
				if (p.getVersionDone != 1) {
					p.getVersionDone = (p.installed == -0.5 || (p.installed == -1 && p.doc.isDisabled1() < 2 && p.doc.isDisabled2() < 2)) ? 0 : 1
				}
			}
		},
		...
		doc: {
			...
			insertHTMLQuery: function () {
				...
				p.insertHTMLQuery1();
				p.insertHTMLQuery2();
				return p.queryObject()
			},
			queryObject: function (s) {
				var t = this,
					r = t.queryObject1(s),
					q = t.queryObject2(s),
					p = r > 0 || q < 0 ? r : (r < -0.5 || q > 0 ? q : (r == -0.1 ? r : 0));
				if (j.dbug) {
					p = r == -0.1 ? r : (!r || !q ? 0 : p)
				}
				t.result = p;
			...
			avoidBrowser: function () {
				var p = j.browser;
				if ((p.isGecko && j.compareNums(p.verGecko, j.formatNum("10")) <= 0 && j.OS <= 4) || (p.isOpera && j.compareNums(p.verOpera, j.formatNum("11")) <= 0 && j.OS <= 4) || (p.isChrome && j.compareNums(p.verChrome, j.formatNum("10")) < 0 && j.OS <= 4)) {
					return 1
				}
				return 0
			},
			hasDummyPDF: function () {
				var p = j.file.getValid(l);
				if (!p || !p.full) {
					return 0
				}
				return 1
			...
			isDisabled1: function () {
				var q = this,
					p = 0;
				if (q.HTML1 || !j.DOM.isEnabled.objectTag()) {
					p = 2
				} else {
					if (j.dbug || j.hasMimeType(q.mimeType)) {} else {
						if (q.avoidBrowser()) {
							p = 2
						}
					}
				}
				if (p < 2 && !q.hasDummyPDF()) {
					p = 1
				}
				return p
			},
			...
			insertHTMLQuery1: function () {
				...
				s = j.DOM.iframe.insert(99, "PDFReader");
				u.DummySpanTagHTML = j.DOM.insert("", [], [], q, p, t, s);
				u.HTML1 = j.DOM.insert("object", (j.browser.isIE && !p.detectIE3P ? ["classid", u.classID] : ["type", u.mimeType]).concat(["data", r]), ["src", r], q, p, t, s);
				u.DummyObjTagHTML1 = j.DOM.insert("object", (j.browser.isIE && !p.detectIE3P ? ["classid", u.classID_dummy] : ["type", u.mimeType_dummy]), [], q, p, t, s);
				j.DOM.iframe.close(s);
				u.queryObject1();
				if (j.browser.isIE && u.result === 0) {
					u.HTML1.span().innerHTML = u.HTML1.outerHTML;
					u.DummyObjTagHTML1.span().innerHTML = u.DummyObjTagHTML1.outerHTML
				}
				p.NOTF.init()
			},
			insertHTMLQuery2: function () {
				...
				r = j.DOM.iframe.insert(99, "PDFReader2");
				t.HTML2 = j.DOM.insert("img", ["alt", q, "src", j.file.getValid(p).full], [], q, p, s, r);
				j.ev.addEvent(t.HTML2.obj(), "load", j.ev.handler(t.onImgLoaded, t));
				j.DOM.iframe.close(r);
				p.NOTF.init()
			},
			onImgLoaded: function (p) {
				p.imgLoaded = 1
			},
			queryObject1: function (r) {
				var t = this,
					p = l,
					q = 0,
					s = 1;
				q = j.DOM.getTagStatus(t.HTML1, t.DummySpanTagHTML, t.DummyObjTagHTML1, 0, null, null, s);
				t.result1 = q;
				return q
			},
			queryObject2: function (r) {
				var s = this,
					p = l,
					q;
				if (s.HTML2.loaded) {
					s.result2 = s.imgLoaded ? 1 : -1
				}
				q = s.result2;
				return q
			}
